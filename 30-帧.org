...
每个帧都有一个默认字体，它指定该帧的默认字符大小。此大小可供以列或行为单位来检索或更改窗口大小（请参阅[[https://github.com/advanceflow/Elisp/blob/main/30-帧.org#30433-尺寸参数][尺寸参数]]）。它也被用于调整窗口大小（请参阅[[https://github.com/advanceflow/Elisp/blob/main/29-窗口.org#294-窗口大小][窗口大小]]）或拆分（请参阅[[https://github.com/advanceflow/Elisp/blob/main/29-窗口.org#297-分割窗口][分割窗口]]）窗口。

有时，术语“行高”（ /line height/ ）和“规范字符高度”（ /canonical character height/ ）也被用于称呼“默认字符高度”。同样，术语“列宽”（ /column width/ ）和“规范字符宽度”（ /canonical character width/ ）也被用于称呼“默认字符宽度”。

- *Function: frame-char-height &optional frame* ::
- *Function: frame-char-width &optional frame* ::

  这些函数返回帧中字符的默认高度和宽度，以像素为单位。它们一起确定了帧默认字体的大小。它们的值取决于所选的帧的字体，详细请参阅[[https://github.com/advanceflow/Elisp/blob/main/30-帧.org#304310-字体和颜色参数][字体和颜色参数]]。

帧的默认字体也可以直接用以下函数设置：

- Command: set-frame-font font &optional keep-size frames ::
  
  该命令将默认字体设置为 /font/ 。当以交互方式调用时，它会提示输入字体的名称，并在当前选中的帧上使用该字体。从 Lisp 调用时，参数 /font/ 应为字体名称（ =font-name= 字符串）、字体对象（ =font-object= ）、字体实体（ =font-entity= ）或字体规范（ =font-spec= ）之一。

  如果可选参数 /keep-size/ 为 =nil= ，帧的行数和列数将保持固定。（如果其值非 =nil= ，下一节中描述的选项 =frame-inhibit-implied-resize= 将覆盖它。）如果 /keep-size/ 非 =nil= （或在交互式调用中使用了前缀参数），则帧内的行数和列数将会进行调整，以使显示区域的大小保持固定。

  如果可选参数 /frames/ 的值为 nil，则仅将字体应用于当前选中的帧。如果 /frames/ 的值非 =nil= ，则它的值应为一个要应用该字体的帧的列表，或者为 =t= 以表示应用于所有现有的和未来的（图形化的）帧。
